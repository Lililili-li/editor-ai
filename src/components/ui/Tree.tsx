...
const Tree: React.FC<TreeProps> = memo(
  ({ onNodeClick, value, treeData, expandAll = true, actionSlot }) => {
      ...
      <>
        {showData.map(
          (item) =>
            item.visible && (
              <div
                className={`tree-item-parent cursor-pointer hover:[&_.action]:opacity-100 hover:[&_.name]:w-[calc(100%-55px)] relative ${
                  value === item.id
              ...
              >
                <div
                  className={`mb-[2px] text-sm text-gray-500 px-2 py-1 rounded-sm group flex justify-between ${
                    value === item.id
                      ? "text-gray-900 dark:text-gray-300"
                      : "dark:text-gray-400 dark:hover:text-gray-100 hover:text-gray-900 overflow-hidden"
                  }`}
                >
                  <div className="flex items-center gap-1 w-full name">
                    {Array.isArray(item?.children) &&
                      item.children.length > 0 && (
                        <Button
                          variant="ghost"
                          className="p-0 h-5 hover:bg-black/4 rounded-4xl w-[20px]"
                          onClick={(e) => {
                            e.stopPropagation();
                            onToggleExpand(item);
                          }}
                    ...
                    <div
                      className={`label text-gray-500 flex items-center gap-1 flex-1 ${
                        value === item.id
                          ? "text-gray-900 dark:text-gray-300"
                    ...
                    >
                      ...
                      <span className="w-full overflow-ellipsis overflow-hidden whitespace-nowrap">
                        {item.title ? item.title : "<无标题>"}
                      </span>
                    </div>
                  </div>
                  <div className="action w-[50px] opacity-0">
                    {actionSlot(item)}
                  </div>
                </div>
              </div>
