...
const ImageResize: FC<ImageResizeProps> = ({
  ...
  const onMouseMove = useCallback(
    (event: MouseEvent) => {
      ...
      const newHeight = moveStateRef.current.height + distance;
      ...
      <Popover>
        <PopoverTrigger asChild>
          ...
          >
            ...
            {editor.isActive("imageBlock") && (
              <div
                className={`mask absolute border-blue-400 border top-0 z-0`}
                style={{
                  background: displayState.moving ? "rgba(0,0,0,.2)" : "",
                  width: displayState.moving
                    ? displayState.width + "px"
                    : "100%",
                  height: displayState.moving
                    ? displayState.height + "px"
                    : "100%",
                }}
              >
                <div
        ...
        <PopoverContent side="top" className="px-2 py-1 w-fit">
          <div className="flex gap-1 justify-between items-center">
            ...
            />
            <AlginImage updateAttributes={updateAttributes} align={align} />
            <Description
              updateAttributes={updateAttributes}
